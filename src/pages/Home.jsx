  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        //create qry
        const qry = query(listingsRef, where('offer', '==', true), orderBy('timestamp', 'desc'), limit(4));
        //execute qry
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const listingsRef = collection(db, 'listings');
        //create qry
        const qry = query(listingsRef, where('type', '==', 'rent'), orderBy('timestamp', 'desc'), limit(4));
        //execute qry
        const qrySnap = await getDocs(qry);
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const listingsRef = collection(db, 'listings');
        //create qry
        const qry = query(listingsRef, where('type', '==', 'sale'), orderBy('timestamp', 'desc'), limit(4));
        //execute qry
        const qrySnap = await getDocs(qry);
